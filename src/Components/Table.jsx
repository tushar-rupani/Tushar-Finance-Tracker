...
export const TableComp = ({dataValue, setDataValue, group}) => {
  ...
  return (
    <div> 
      {group === false && <input type='text' placeholder='Enter Search..' onChange={handleSearch} style={{ margin: "20px", padding: "10px" }} /> }
      {dataFound}
          ...
          <TableHead style={{ background: "rgb(160, 162, 192)", color: "white", cursor: "pointer" }}>
            <TableRow>
              {transactionHeader.map((header, index) => (
                <TableCell key={index} align='right' onClick={() => handleSort(header.functionTitle)}> <span>{header.title}</span><SortIcon />
                </TableCell>
              ))}
